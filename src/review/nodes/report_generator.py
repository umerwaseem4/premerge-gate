...
def generate_markdown_report(state: ReviewState, artifact_url: Optional[str] = None) -> str:
    ...
    # Header with emoji based on decision
    if decision == "PASS":
        header = "## ✅ AI Staff Review — Passed"
    elif decision == "FAIL":
        header = "## ❌ AI Staff Review — Failed"
    else:
        header = "## ⚠️ AI Staff Review — Error"
    ...
    # Non-blocking issues
    if non_blocking:
        lines.append(f"### ⚠️ Non-Blocking Issues ({len(non_blocking)})")
        lines.append("")
...
def generate_docx_report(state: ReviewState) -> bytes:
    ...
    # Title
    title = doc.add_heading("AI Code Review Report", 0)
    ...
    footer.add_run(
        "This report was generated by the AI Staff Review system. "
...
def save_docx_report(state: ReviewState, output_dir: str = "reports") -> str:
    ...
    filename = f"ai_review_pr{pr_num}_{timestamp}.docx"
