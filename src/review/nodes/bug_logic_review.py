...
SYSTEM_PROMPT = """You are a senior engineer reviewing code for correctness issues.
...
Only report issues you're confident about. Severity guide:
- BLOCKING: Will cause bugs, crashes, or security issues
- NON_BLOCKING: Should be fixed but won't break production
...
async def bug_logic_review(state: ReviewState, llm: ChatOpenAI) -> Dict:
    ...
    user_message = f"""
Review this code diff for correctness issues:
